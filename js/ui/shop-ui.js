...
export function renderShop() {
  ...
  ctx.lineWidth = 1;
  ctx.strokeRect(0.5, 0.5, CANVAS_WIDTH - 1, 71);
  ...
  ctx.fillText(shop.name, CANVAS_WIDTH / 2, 8);
  ...
  if (shop.type !== 'spell_shop') {
    ...
    ctx.fillText('BUY', CANVAS_WIDTH / 2 - 60, 44);
    ctx.fillStyle = c.textDim;
    ctx.fillText('/', CANVAS_WIDTH / 2, 44);
    ctx.fillStyle = sellColor;
    ctx.fillText('SELL', CANVAS_WIDTH / 2 + 60, 44);
  }
  ctx.textAlign = 'left';
  ...
  ctx.textAlign = 'right';
  ctx.fillText(`Gold: ${player.gold}G`, CANVAS_WIDTH - PAD, 24);
  ctx.textAlign = 'left';
  ...
  ctx.font = '18px monospace';
  ctx.fillStyle = c.textDim;
  ctx.fillText('STATS', statsX + 20, statsY + 20);
  ...
  drawBar(ctx, sx + 44, sy - 4, statsW - 140, 20, player.hp, player.maxHp, COLORS.hud.hp, COLORS.hud.hpBg);
  ctx.fillText(`${player.hp}/${player.maxHp}`, sx + statsW - 88, sy);
  sy += 36;

  // MP bar
  ctx.fillText('MP', sx, sy);
  drawBar(ctx, sx + 44, sy - 4, statsW - 140, 20, player.mp, player.maxMp, COLORS.hud.mp, COLORS.hud.mpBg);
  ctx.fillText(`${player.mp}/${player.maxMp}`, sx + statsW - 88, sy);
  sy += 48;

  // Stat values
  ctx.fillStyle = c.textDim;
  ctx.font = '18px monospace';
  ctx.fillText(`ATK  ${player.atk}`, sx, sy); sy += 32;
  ctx.fillText(`DEF  ${player.def}`, sx, sy); sy += 32;
  ctx.fillText(`SPD  ${player.spd}`, sx, sy); sy += 32;
  ctx.fillText(`LCK  ${player.lck}`, sx, sy); sy += 32;
  ctx.fillText(`INT  ${player.int || 2}`, sx, sy); sy += 48;

  // Controls hint
  ctx.fillStyle = c.textDim;
  ctx.font = '16px monospace';
  ctx.fillText('↑↓ Browse', sx, statsY + statsH - 92);
  const actionHint = shop.mode === 'sell' ? 'Enter Sell' : 'Enter Buy';
  ctx.fillText(actionHint, sx, statsY + statsH - 64);
  if (shop.type !== 'spell_shop') {
    ctx.fillText('Tab Buy/Sell', sx, statsY + statsH - 36);
  }
  ctx.fillText('Esc Exit', sx + 160, statsY + statsH - 64);
}

function renderBuyMode(ctx, shop, player, c, listX, listY, listW, listH) {
  ctx.font = '18px monospace';
  ctx.fillStyle = c.textDim;
  ctx.fillText(shop.type === 'spell_shop' ? 'SPELLS' : 'ITEMS', listX + 20, listY + 20);

  for (let i = 0; i < shop.items.length; i++) {
    const item = shop.items[i];
    const iy = listY + 60 + i * LINE_H;
    const canAfford = player.gold >= item.cost;

    if (item.isSpell) {
      if (i === shop.menuIndex) {
        ctx.fillStyle = 'rgba(255, 215, 0, 0.1)';
        ctx.fillRect(listX + 4, iy - 8, listW - 8, LINE_H);
      }
      ...
      ctx.fillText(`${prefix}${item.name}`, listX + 20, iy);

      ctx.textAlign = 'right';
      if (item.alreadyKnown) {
        ctx.fillStyle = c.bought;
        ctx.fillText('Learned', listX + listW - 20, iy);
      } else if (item.levelLocked) {
        ctx.fillStyle = c.noGold;
        ctx.fillText(`Lv ${item.learnLevel}`, listX + listW - 20, iy);
      } else {
        ctx.fillStyle = canAfford ? c.gold : c.noGold;
        ctx.fillText(`${item.cost}G`, listX + listW - 20, iy);
      }
      ctx.textAlign = 'left';
    } else {
      if (i === shop.menuIndex) {
        ...
        ctx.fillStyle = c.menuSelect;
        ctx.font = '20px monospace';
        ctx.fillText(`▸ ${item.name}`, listX + 20, iy);
      } else {
        ctx.fillStyle = canAfford ? c.menuNormal : c.textDim;
        ctx.font = '20px monospace';
        ctx.fillText(`  ${item.name}`, listX + 20, iy);
  ...
  if (selected) {
    const descY = listY + 60 + shop.items.length * LINE_H + 32;
    ctx.fillStyle = c.panelBorder;
    ctx.fillRect(listX + 16, descY - 8, listW - 32, 1);
    ...
    ctx.font = '18px monospace';
    ctx.fillText(selected.desc, listX + 20, descY + 16);
...
function renderSellMode(ctx, shop, player, c, listX, listY, listW, listH) {
  ctx.font = '18px monospace';
  ctx.fillStyle = c.textDim;
  ctx.fillText('YOUR ITEMS', listX + 20, listY + 20);
  ...
  if (items.length === 0) {
    ctx.fillStyle = c.textDim;
    ctx.font = '18px monospace';
    ctx.fillText('Nothing to sell', listX + 20, listY + 72);
    return;
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (i === shop.menuIndex) {
      ...
      ctx.fillText(label, listX + 20, iy);
    } else {
    ...
    ctx.fillText(item.sellPrice > 0 ? `${item.sellPrice}G` : '---', listX + listW - 20, iy);
    ctx.textAlign = 'left';
  }
  ...
  if (selected) {
    const descY = listY + 60 + items.length * LINE_H + 32;
    ctx.fillStyle = c.panelBorder;
    ctx.fillRect(listX + 16, descY - 8, listW - 32, 1);
    ctx.fillStyle = c.textDim;
    ctx.font = '18px monospace';
